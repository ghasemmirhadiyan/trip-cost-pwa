...
window.showPage=async function showPage(page){let title='',body='';
 ...
 } else if(page==='fund'){title='🏦 صندوق';body=`<div id="fundLive"><div class="stat primary"><span>موجودی فعلی صندوق</span><strong>در حال محاسبه...</strong><small>تومان</small></div><div class="stat" style="margin-top:10px"><span>📥 مطالبات صندوق</span><strong>در حال محاسبه...</strong><small>تومان</small></div></div><button class="btn" onclick="newContribution()">➕ ثبت واریزی به صندوق</button><div id="contributionList"><div class="list-item"><b>تراکنش‌های صندوق</b><p class="muted">در حال بارگذاری...</p></div></div>`;setTimeout(async()=>{const f=await loadFinancialSummary();const t=f?.trip||{};const el=document.querySelector('#fundLive');if(el)el.innerHTML=`<div class="stat primary"><span>موجودی فعلی صندوق</span><strong>${money(t.current_fund_balance)}</strong><small>تومان</small></div><div class="stat" style="margin-top:10px"><span>📥 مطالبات صندوق</span><strong>${money(Math.max(0,Number(t.current_fund_balance||0)+Number(t.approved_contributions||0)-Number(t.approved_expenses||0)))}</strong><small>تومان</small></div>`;const rows=await loadContributions();const lm=document.querySelector('#contributionList');if(lm)lm.innerHTML=rows.map(c=>{const m=state.members.find(x=>Number(x.id)===Number(c.trip_member_id));const cls=c.status==='approved'?'approved':c.status==='rejected'?'danger':'pending';const label=statusFa(c.status);const actions=window.authState?.member?.role==='admin'&&c.status==='pending'?`<div class="actions"><button class="btn small" onclick="approveContribution('${c.id}')">✓ تأیید</button><button class="btn danger small" onclick="rejectContribution('${c.id}')">× رد</button></div>`:'';return `<div class="list-item"><span class="badge ${cls}">${label}</span><b>📥 ${escapeHtml(m?.name||'عضو')}</b><p>${money(c.amount)} تومان • ${c.method==='cash'?'نقدی':c.method==='card'?'کارت':c.method==='bank_transfer'?'انتقال بانکی':'سایر'}</p><small>تاریخ: ${c.contribution_date||''}</small>${c.note?`<p>${escapeHtml(c.note)}</p>`:''}${actions}</div>`}).join('')||'<div class="empty-state">هنوز واریزی ثبت نشده است.</div>';},0);
 } else if(page==='members'){await loadTripSettings();await loadTripMembers();title='👥 اعضای سفر';body=`<div class="stat primary"><span>💰 مبلغ هر سهم</span><strong>${money(state.shareAmount)}</strong><small>تعهد هر عضو = تعداد سهم × این مبلغ</small></div>${window.authState?.member?.role==='admin'?`<button class="btn" onclick="editShareAmount()">✏️ تغییر مبلغ هر سهم</button>`:''}<button type="button" class="btn" data-action="add-member">➕ ساخت حساب و افزودن عضو</button><p class="muted">تعداد سهم هر عضو از قبل مشخص می‌شود و مبلغ تعهد صندوق به‌صورت خودکار محاسبه می‌شود.</p><h3>اعضای فعال</h3>${state.members.map(m=>`<div class="list-item"><b>${escapeHtml(m.name)}</b><p>${Number(m.share_weight)} سهم • تعهد صندوق: ${money(m.contribution_target)}</p><small>${m.role==='admin'?'👑 مدیر سفر':'👤 عضو'}</small>${window.authState?.member?.role==='admin'&&m.user_id!==window.authState?.session?.user?.id?`<div class="actions"><button class="btn danger small" onclick="deleteMember('${m.id}','${escapeAttr(m.name)}')">🗑️ حذف عضو</button></div>`:''}</div>`).join('')||'<p class="muted">عضوی وجود ندارد.</p>'}`;
