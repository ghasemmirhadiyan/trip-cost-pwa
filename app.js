...
const state={role:'admin',user:'قاسم',trip:'مدیریت سفر گروهی',pendingMembers:[],members:[],expenses:[],locations:[],itinerary:[],shareAmount:12000000,settlementEnabled:false};
...
window.showPage=async function showPage(page){let title='',body='';
 ...
 } else if(page==='about'){title='ℹ️ درباره برنامه';body=`<div class="about-card"><div class="about-logo">🌲</div><div class="about-kicker">برای یک سفر خوب، با هم آماده شویم</div><h3>همه‌چیز برای یک سفر گروهی بهتر</h3><p>این برنامه برای مدیریت ساده و شفاف هزینه‌ها، صندوق سفر، اعضا، مکان‌های پیشنهادی، برنامه سفر و خاطرات تصویری طراحی شده است تا همه اعضای گروه اطلاعات سفر را یکجا ببینند و هماهنگ باشند.</p><div class="about-features"><span>💰 مدیریت هزینه</span><span>🏦 صندوق مشترک</span><span>🗺️ برنامه سفر</span><span>📷 آلبوم خاطرات</span></div><div class="creator-card"><div class="creator-avatar">ق</div><div><small>سازنده و مدیر برنامه</small><strong>قاسم میرهادیان</strong><p>طراحی و توسعه با هدف ساده‌تر شدن مدیریت سفرهای گروهی</p></div></div><div class="about-version">نسخه برنامه: <strong>15.3</strong></div><div class="about-footer">با آرزوی سفری شاد، آرام و پر از خاطرات خوب ❤️</div></div>`;
...
} else if(page==='admin'){if(window.authState?.member?.role!=='admin'){return alert('فقط مدیر سفر به پنل مدیریت دسترسی دارد.');}title='👑 پنل مدیریت';body=`<div class="admin-grid"><button onclick="showPage('members')">👥<b>اعضا</b><small>مدیریت اعضا</small></button><button onclick="showPage('pending')">🟡<b>تأیید هزینه‌ها</b><small>بررسی هزینه‌های جدید</small></button><button onclick="showPage('fund')">🏦<b>تراکنش‌های صندوق</b><small>ویرایش و تأیید واریزی‌ها</small></button><button onclick="manageAnnouncement()">📢<b>پیام اعضا</b><small>پیام پاپ‌آپ هنگام ورود</small></button><button>📊<b>گزارش‌ها</b><small>Excel / PDF</small></button></div><div class="list-item"><b>📢 پیام آغاز برنامه</b><p class="muted">یک پیام برای اعضا تنظیم کنید تا پس از ورود به برنامه به‌صورت پاپ‌آپ نمایش داده شود.</p><button class="btn" onclick="manageAnnouncement()">✏️ تنظیم پیام</button></div>`;
 } else {title='☰ امکانات بیشتر';body=`<div class="list-item install-menu-item" onclick="installPWA()"><b>📲 نصب برنامه روی اندروید</b><p>نصب مستقیم روی صفحه اصلی موبایل</p></div>`+[['📷 آلبوم عکس','album'],['🎒 چک‌لیست سفر','checklist'],['🔔 اعلان‌ها','notifications'],['📊 گزارش‌ها و نمودارها','reports'],['⚙️ تنظیمات','settings']].map(([x,p])=>`<div class="list-item" onclick="showPage('${p}')"><b>${x}</b></div>`).join('')+`<div class="list-item about-menu-item" onclick="showPage('about')"><b>ℹ️ درباره برنامه</b><p>معرفی برنامه و سازنده</p></div>`+(window.authState?.member?.role==='admin'?`<div class="list-item" onclick="showPage('admin')"><b>👑 پنل مدیریت</b><p>مدیریت اعضا، تأییدها و سفرها</p></div>`:'');}
...
window.deleteExpense=async(id)=>{if(window.authState?.member?.role!=='admin')return alert('فقط مدیر سفر می‌تواند هزینه را حذف کند.');const e=state.expenses.find(x=>x.id===id);if(!e)return alert('هزینه پیدا نشد.');if(!confirm(`آیا از حذف این هزینه مطمئن هستید؟\n\n${e.title} — ${money(e.amount)} تومان\nوضعیت: ${statusFa(e.status)}\n\nاگر هزینه تأییدشده باشد، حذف آن بلافاصله در محاسبات مالی اعمال می‌شود.`))return;const {error}=await window.sb.rpc('delete_expense_admin',{p_expense_id:id});if(error){alert('حذف هزینه انجام نشد:\n'+error.message);return;}await loadExpenses();await showPage('expenses');alert('هزینه با موفقیت حذف شد.');};
...
window.loadContributions=async()=>{
 if(!window.authState?.tripId)return [];
 const {data,error}=await window.sb.from('fund_contributions').select('id,trip_member_id,amount,method,contribution_date,status,submitted_by,note,rejection_reason,created_at,updated_at').eq('trip_id',window.authState.tripId).order('created_at',{ascending:false});
 if(error){console.error('contributions',error);return [];} return data||[];
};
window.editContribution=async(id)=>{
 if(window.authState?.member?.role!=='admin')return alert('فقط مدیر سفر می‌تواند تراکنش صندوق را ویرایش کند.');
 const rows=await loadContributions(); const c=rows.find(x=>String(x.id)===String(id)); if(!c)return alert('تراکنش پیدا نشد.');
 await loadTripMembers();
 const opts=state.members.map(m=>`<option value="${m.id}" ${Number(m.id)===Number(c.trip_member_id)?'selected':''}>${escapeHtml(m.name)}</option>`).join('');
 modal().innerHTML=`<div class="sheet"><button class="close" onclick="closeModal()">×</button><h2>✏️ ویرایش تراکنش صندوق</h2><p class="muted">وضعیت «${statusFa(c.status)}» حفظ می‌شود؛ فقط اطلاعات تراکنش تغییر می‌کند.</p><div class="form"><label>واریزکننده<select id="efcMember">${opts}</select></label><label>مبلغ (تومان)<input id="efcAmount" type="number" min="1" value="${Number(c.amount)||0}"></label><label>تاریخ واریز<input id="efcDate" type="date" value="${c.contribution_date||''}"></label><label>روش پرداخت<select id="efcMethod"><option value="cash" ${c.method==='cash'?'selected':''}>نقدی</option><option value="card" ${c.method==='card'?'selected':''}>کارت</option><option value="bank_transfer" ${c.method==='bank_transfer'?'selected':''}>کارت به کارت / انتقال بانکی</option><option value="other" ${c.method==='other'?'selected':''}>سایر</option></select></label><label>توضیحات<textarea id="efcNote">${escapeHtml(c.note||'')}</textarea></label><button class="btn" onclick="saveEditedContribution('${c.id}')">💾 ذخیره تغییرات</button></div></div>`; modal().classList.remove('hidden');
};
window.saveEditedContribution=async(id)=>{
 const memberId=Number($('#efcMember')?.value||0), amount=Number($('#efcAmount')?.value||0), date=$('#efcDate')?.value||null, method=$('#efcMethod')?.value||'cash', note=$('#efcNote')?.value.trim()||null;
 if(!memberId||!amount||amount<=0||!date)return alert('اطلاعات تراکنش را کامل و صحیح وارد کنید.');
 const {error}=await window.sb.rpc('update_fund_contribution_admin',{p_contribution_id:id,p_trip_member_id:memberId,p_amount:Math.round(amount),p_method:method,p_contribution_date:date,p_note:note});
 if(error){alert('ویرایش تراکنش انجام نشد:\n'+error.message);return;}
 closeModal(); await showPage('fund'); alert('تراکنش صندوق با موفقیت ویرایش شد.');
};
window.deleteContribution=async(id)=>{
 if(window.authState?.member?.role!=='admin')return alert('فقط مدیر سفر می‌تواند تراکنش صندوق را حذف کند.');
 const rows=await loadContributions();
 const c=rows.find(x=>String(x.id)===String(id));
 if(!c)return alert('تراکنش پیدا نشد.');
 const member=state.members.find(m=>Number(m.id)===Number(c.trip_member_id));
 const ok=confirm(`آیا از حذف این تراکنش مطمئن هستید؟\n\n${member?.name||'عضو'} — ${money(c.amount)} تومان\nوضعیت: ${statusFa(c.status)}\n\nاگر تراکنش تأییدشده باشد، حذف آن بلافاصله موجودی و محاسبات صندوق را نیز تغییر می‌دهد.`);
 if(!ok)return;
 const {error}=await window.sb.rpc('delete_fund_contribution_admin',{p_contribution_id:id});
 if(error){alert('حذف تراکنش انجام نشد:\n'+error.message);return;}
 closeModal(); await showPage('fund'); alert('تراکنش صندوق با موفقیت حذف شد.');
};
