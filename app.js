...
window.showPage=async function showPage(page){let title='',body='';
 ...
 if(page==='expenses'||page==='pending'||page==='approved'||page==='rejected'){title=page==='pending'?'🟡 هزینه‌های در انتظار تأیید':page==='approved'?'🟢 هزینه‌های تأیید شده':page==='rejected'?'🔴 هزینه‌های رد شده':'💰 هزینه‌ها'; const arr=page==='pending'?state.expenses.filter(e=>e.status==='pending'):page==='approved'?state.expenses.filter(e=>e.status==='approved'):page==='rejected'?state.expenses.filter(e=>e.status==='rejected'):state.expenses; body=`<div class="filter-row"><button class="chip ${page==='expenses'?'active':''}" onclick="showPage('expenses')">همه</button><button class="chip ${page==='pending'?'active':''}" onclick="showPage('pending')">🟡 در انتظار</button><button class="chip ${page==='approved'?'active':''}" onclick="showPage('approved')">🟢 تأیید شده</button><button class="chip ${page==='rejected'?'active':''}" onclick="showPage('rejected')">🔴 رد شده</button></div>${arr.map(e=>{const payer=state.members.find(m=>m.id===e.payer_member_id);return `<div class="list-item"><span class="badge ${e.status==='pending'?'pending':e.status==='approved'?'approved':'danger'}">${statusFa(e.status)}</span><b>${escapeHtml(e.title)}</b><p>${e.from_fund?'🏦 صندوق':escapeHtml(payer?.name||'پرداخت‌کننده')} • ${money(e.amount)}</p><small>تاریخ: ${e.expense_date||''}</small>${window.authState?.member?.role==='admin'?`<div class="actions"><button class="btn small" onclick="editExpense('${e.id}')">✏️ ویرایش</button>${e.status==='pending'?`<button class="btn small" onclick="approveExpense('${e.id}')">✓ تأیید</button><button class="btn danger small" onclick="rejectExpense('${e.id}')">× رد</button>`:''}</div>`:''}</div>`}).join('')||'<div class="empty-state">موردی برای نمایش وجود ندارد.</div>'}<button class="btn" onclick="newExpense()">➕ ثبت هزینه جدید</button>`; setTimeout(loadTripMembers,0); setTimeout(loadExpenses,0);
   ...
   let counts={members:0,expenses:0,contributions:0,locations:0,itinerary:0};
   if(isAdmin){
     ...
     counts={members:mr.count||0,expenses:ex.count||0,contributions:fc.count||0,locations:lo.count||0,itinerary:it.count||0};
   }
   const total=Object.values(counts).reduce((a,b)=>a+b,0);
   body=`<div class="notification-hero"><div class="notification-icon">🔔</div><h3>${isAdmin?'اعلان‌های مدیریتی':'اعلان‌ها'}</h3><p>${isAdmin?(total?`${total.toLocaleString('fa-IR')} درخواست در انتظار بررسی مدیر است.`:'در حال حاضر درخواست تأییدنشده‌ای وجود ندارد.'):'در این بخش اعلان‌های مربوط به سفر نمایش داده می‌شود.'}</p></div>`;
   if(isAdmin) body+=`<div class="notification-list">
     <div class="list-item" onclick="showPage('members')"><b>👤 درخواست‌های عضویت</b><span class="badge ${counts.members?'pending':'approved'}">${counts.members}</span><p>درخواست‌های جدید برای تأیید عضو</p></div>
     <div class="list-item" onclick="showPage('pending')"><b>💰 هزینه‌های جدید</b><span class="badge ${counts.expenses?'pending':'approved'}">${counts.expenses}</span><p>هزینه‌های در انتظار تأیید</p></div>
     <div class="list-item" onclick="showPage('fund')"><b>🏦 واریزی‌های صندوق</b><span class="badge ${counts.contributions?'pending':'approved'}">${counts.contributions}</span><p>واریزی‌های در انتظار تأیید</p></div>
     <div class="list-item" onclick="showPage('locations')"><b>📍 پیشنهاد مکان</b><span class="badge ${counts.locations?'pending':'approved'}">${counts.locations}</span><p>مکان‌های پیشنهادی در انتظار تأیید</p></div>
     <div class="list-item" onclick="showPage('itinerary')"><b>🗺️ پیشنهاد برنامه سفر</b><span class="badge ${counts.itinerary?'pending':'approved'}">${counts.itinerary}</span><p>برنامه‌های پیشنهادی در انتظار تأیید</p></div>
   </div>`;
   body+=`<button class="btn" onclick="showPage('home')">🏠 بازگشت به صفحه اصلی</button>`;
 ...
 } else {title='☰ امکانات بیشتر';body=`<div class="list-item install-menu-item" onclick="installPWA()"><b>📲 نصب برنامه روی اندروید</b><p>نصب مستقیم روی صفحه اصلی موبایل</p></div>`+['📷 آلبوم عکس','🎒 چک‌لیست سفر','🔔 اعلان‌ها','📊 گزارش‌ها و نمودارها','⚙️ تنظیمات'].map(x=>`<div class="list-item" ${x.startsWith('📷')?'onclick="showPage(\'album\')"':x.startsWith('🔔')?'onclick="showPage(\'notifications\')"':''}><b>${x}</b></div>`).join('')+`<div class="list-item about-menu-item" onclick="showPage('about')"><b>ℹ️ درباره برنامه</b><p>معرفی برنامه و سازنده</p></div>`+(state.role==='admin'?`<div class="list-item" onclick="showPage('admin')"><b>👑 پنل مدیریت</b><p>مدیریت اعضا، تأییدها و سفرها</p></div>`:'');}
