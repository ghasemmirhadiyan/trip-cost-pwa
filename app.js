...
window.showPage=async function showPage(page){let title='',body='';
 ...
 } else if(page==='members'){await loadTripSettings();await loadTripMembers();const fin=await loadFinancialSummary();const fm=Object.fromEntries((fin?.members||[]).map(x=>[String(x.trip_member_id),x]));title='👥 اعضای سفر';body=`<div class="stat primary"><span>💰 مبلغ هر سهم</span><strong>${money(state.shareAmount)}</strong><small>تعهد صندوق هر عضو بر اساس ضریب ثابت سفر محاسبه می‌شود</small></div>${window.authState?.member?.role==='admin'?`<button class="btn" onclick="editShareAmount()">✏️ تغییر مبلغ هر سهم</button>`:''}<button type="button" class="btn" data-action="add-member">➕ ساخت حساب و افزودن عضو</button><p class="muted">بدهی یا طلب صندوق بر اساس واریزی‌های تأییدشده محاسبه می‌شود.</p>${state.members.map(m=>{const f=fm[String(m.id)]||{};const target=Number(m.contribution_target||f.contribution_target||0);const paid=Number(f.approved_contributions||0);const diff=paid-target;const balanceHtml=diff<0?`<span class="fund-debt">🔴 بدهی به صندوق: ${money(-diff)}</span>`:diff>0?`<span class="fund-credit">🟢 طلب از صندوق: ${money(diff)}</span>`:`<span class="fund-settled">⚪ تسویه شده</span>`;return `<div class="list-item member-item"><div class="member-head"><div class="mini-avatar">${m.avatar_url?`<img src="${escapeAttr(m.avatar_url)}" alt="">`:(escapeHtml((m.name||'ع').slice(0,1)))}</div><b>${escapeHtml(m.name)}</b></div><div class="member-finance"><div><small>تعهد صندوق</small><strong>${money(target)}</strong></div><div>${balanceHtml}</div></div><small>${m.role==='admin'?'👑 مدیر سفر':'👤 عضو'}</small>${window.authState?.member?.role==='admin'?`<div class="actions"><button class="btn small" onclick="editMember('${m.id}')">✏️ ویرایش پروفایل</button>${m.user_id!==window.authState?.session?.user?.id?`<button class="btn danger small" onclick="deleteMember('${m.id}','${escapeAttr(m.name)}')">🗑️ حذف</button>`:''}</div>`:''}</div>`}).join('')||'<p class="muted">عضوی وجود ندارد.</p>'}`;
