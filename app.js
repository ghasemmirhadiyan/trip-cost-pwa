...
const state={role:'admin',user:'قاسم',trip:'سفر شمال ۱۴۰۵',pendingMembers:[],members:[],expenses:[],locations:[]};
...
window.showPage=async function showPage(page){let title='',body='';
 ...
 } else if(page==='fund'){title='🏦 صندوق';body=`<div id="fundLive"><div class="stat primary"><span>موجودی فعلی صندوق</span><strong>در حال محاسبه...</strong><small>تومان</small></div><div class="stat" style="margin-top:10px"><span>📥 مطالبات صندوق</span><strong>در حال محاسبه...</strong><small>تومان</small></div></div><button class="btn" onclick="newContribution()">➕ ثبت واریزی به صندوق</button><div class="list-item"><b>تراکنش‌های اخیر</b><p class="muted">تراکنش واقعی پس از ثبت و تأیید نمایش داده می‌شود.</p></div>`;setTimeout(async()=>{const f=await loadFinancialSummary();const t=f?.trip||{};const el=document.querySelector('#fundLive');if(el)el.innerHTML=`<div class="stat primary"><span>موجودی فعلی صندوق</span><strong>${money(t.current_fund_balance)}</strong><small>تومان</small></div><div class="stat" style="margin-top:10px"><span>📥 مطالبات صندوق</span><strong>${money(Math.max(0,Number(t.current_fund_balance||0)+Number(t.approved_contributions||0)-Number(t.approved_expenses||0)))}</strong><small>تومان</small></div>`;},0);
 } else if(page==='members'){await loadTripMembers();title='👥 اعضای سفر';body=`<button type="button" class="btn" data-action="add-member">➕ ساخت حساب و افزودن عضو</button><p class="muted">ادمین می‌تواند حساب بسازد، عضو اضافه کند یا عضو را از سفر حذف کند.</p><h3>اعضای فعال</h3>${state.members.map(m=>`<div class="list-item"><b>${escapeHtml(m.name)}</b><p>سهم هدف صندوق: ${money(m.contribution_target)}</p><small>${m.role==='admin'?'👑 مدیر سفر':'👤 عضو'}</small>${window.authState?.member?.role==='admin'&&m.user_id!==window.authState?.session?.user?.id?`<div class="actions"><button class="btn danger small" onclick="deleteMember('${m.id}','${escapeAttr(m.name)}')">🗑️ حذف عضو</button></div>`:''}</div>`).join('')||'<p class="muted">عضوی وجود ندارد.</p>'}`;
