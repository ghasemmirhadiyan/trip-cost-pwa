...
window.showPage=async function showPage(page){let title='',body='';
 ...
 } else if(page==='members'){await loadTripMembers();title='👥 اعضای سفر';body=`<button type="button" class="btn" data-action="add-member">➕ ساخت حساب و افزودن عضو</button><p class="muted">در این سفر فقط ادمین می‌تواند حساب جدید بسازد و عضو را مستقیم اضافه کند.</p><h3>اعضای فعال</h3>${state.members.map(m=>`<div class="list-item"><b>${escapeHtml(m.name)}</b><p>ضریب مشارکت: ${m.share_weight} • سهم صندوق: ${money(m.contribution_target)}</p><small>${m.role==='admin'?'👑 مدیر سفر':'👤 عضو'}</small></div>`).join('')||'<p class="muted">عضوی وجود ندارد.</p>'}`;
...
} else if(page==='admin'){title='👑 پنل مدیریت';body=`<div class="admin-grid"><button onclick="showPage('members')">👥<b>اعضا</b><small>عضویت و ضرایب</small></button><button onclick="showPage('pending')">🟡<b>تأیید هزینه‌ها</b><small>بررسی هزینه‌های جدید</small></button><button>📊<b>گزارش‌ها</b><small>Excel / PDF</small></button></div>`;
